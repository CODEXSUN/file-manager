...
const createStorageConnectionsTable = `CREATE TABLE IF NOT EXISTS fm_storage_connections (
...
export const storageConnectionMigration = {
  checksum: createStorageConnectionsTable,
  description: "Create tenant-scoped encrypted storage connections.",
  key: "file-manager.storage-connection.v1",
  async run() {
    await sql
      .raw(createStorageConnectionsTable)
